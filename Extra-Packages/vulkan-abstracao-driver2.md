...
## Apêndice — A saga do benchmark e a autocorreção

> Esta seção é deliberadamente um registro de **método**, não de Vulkan. Ela
> documenta um erro real que foi cometido e corrigido durante o diagnóstico,
> porque a lição vale mais que o número final.

### O benchmark contaminado

A primeira comparação CPU vs GPU foi feita assim:

```bash
ollama run --verbose qwen3:4b "..."                  # suposta GPU
OLLAMA_VULKAN=0 ollama run --verbose qwen3:4b "..."  # supostamente CPU
```

Resultado, geração: **17.70 tok/s** (GPU) vs **17.71 tok/s** (CPU). Empate
perfeito. A conclusão tirada na hora foi: *"a geração é limitada por banda de
memória (LPDDR5 compartilhada), GPU e CPU empatam — desliga o Vulkan, a CPU
basta."*

**Essa conclusão estava errada.** E o erro tinha uma causa concreta:

> `OLLAMA_VULKAN=0 ollama run` seta a variável no **cliente** (`ollama run`),
> mas a inferência roda no **daemon** (`ollama serve`). O daemon, configurado
> pelo NixOS, continuou com `OLLAMA_VULKAN:true`.

Ou seja: **as duas execuções rodaram na GPU.** Os 17.70 e 17.71 não eram
"GPU vs CPU" — eram a mesma coisa medida duas vezes. O empate perfeito não era
empate; era identidade.

### A prova do erro

A contaminação foi confirmada pelo log de `server config`, que lista o env que
o daemon realmente parseou. Em **todas** as execuções, inclusive as do
benchmark:
...
O cliente dizer `0` não muda o daemon. Lição crua: **variável de cliente ≠
variável de daemon.** Num modelo cliente-servidor, medir o backend exige
controlar o *servidor*, não o comando que dispara a requisição.

### O teste limpo

Para medir a CPU de verdade, a GPU teve que ser desligada **no daemon**, o que
no NixOS significou rebuild (o `systemctl edit` falha — `/etc` é read-only; e o
`ollama serve` manual esbarrou em permissão do diretório de modelos). Config de
teste:
...
Confirmado no log antes de medir — esta verificação é o passo que não pode
faltar:
...
E a prova por requisição: `offloaded 0/37 layers` (nada na GPU).

### Os números reais

| Métrica | GPU (Vulkan) | CPU pura (limpa) | Vantagem GPU |
|---------|--------------|------------------|--------------|
| Geração (eval rate) | ~17–19 tok/s | **3.51 tok/s** | **~5x** |
| Prefill (prompt eval)* | ~59–65 tok/s | ~15 tok/s | ~4x |

\* *Ressalva: prefill medido com prompt de 20–23 tokens é ruído — o tempo é
dominado por overhead fixo, não por throughput. Os números de prefill variaram
de 59 a 288 tok/s entre runs idênticas de GPU. Só vale com prompt longo
(500+ tokens). A geração, medida sobre ~1000–2400 tokens, é o número confiável.*

### Conclusão corrigida

O veredito inicial ("CPU basta, desliga o Vulkan") foi **retirado**. Com dado
limpo dos dois lados, a iGPU Arc entrega **~5x** a CPU na geração. Decisão final:
**manter o Vulkan ligado.** A "CPU lenta" (3.51 tok/s) ficou como curiosidade
em aberto — provavelmente threading/backend subótimo — mas sem impacto prático,
já que a GPU é o caminho.

### As três lições de método

1. **Variável de cliente ≠ de daemon.** Em arquitetura cliente-servidor, mede-se
   o servidor. Verificar o `server config` antes de confiar em qualquer número.

2. **Provar o backend por execução.** Não basta "achar" que está na GPU/CPU —
   confirmar `offloaded N/N` em cada run. `37/37` = GPU; `0/37` = CPU.

3. **Amostra pequena mente.** Prefill com ~20 tokens é variância pura. Throughput
   só é confiável com amostra grande (geração de centenas/milhares de tokens).

### Sobre a autocorreção

O erro original — cravar "CPU vence" sobre dado contaminado — veio de confiar em
um **modelo mental** (a teoria do gargalo de banda de memória) em vez de exigir
a medição limpa. O mesmo padrão se repetiu duas vezes na sessão: chamar
`OLLAMA_IGPU_ENABLE` de "variável fantasma" (era real — o log provou) e propor
`OLLAMA_NUM_THREADS` (não existe — o `server config` desmentiu antes do rebuild).

O fio condutor da correção foi sempre o mesmo: **parar de teorizar e ir à
fonte** — o log de `server config`, o `offloaded N/N`, o `--help`, o
`VK_LOADER_DEBUG`. Teoria boa aplicada a dado errado produz conclusão errada com
confiança alta, que é o pior tipo de erro. A disciplina que salvou o diagnóstico
foi a insistência em verificar cada afirmação no próprio sistema antes de cravar.
